document.addEventListener('DOMContentLoaded', function () {
            ...
            if (userData.friends && userData.friends.length > 0) {
                ...
                db.collection('tasks').doc(userId).get().then((taskDoc) => {
                    if (taskDoc.exists) {
                        ...
                        displayTask(taskData, userId, friendId);
                    } else {
                        fetch('assets/js/java_exercises.json')
                            ...
                            .then(exercises => {
                                ...
                                displayTask(randomExercise, userId, friendId);
                            });
                    }
                });
            }
    ...
    function displayTask(task, userId, friendId) {
        ...
        initializeEditors(task.code_template, userId, friendId);
    }

    function initializeEditors(codeTemplate, userId, friendId) {
        ...
        require(['vs/editor/editor.main'], function () {
            ...
            runBtn.addEventListener('click', async function () {
                ...
                try {
                    ...
                    notifyFriend(userId, friendId, won);
            ...
            function notifyFriend(winnerId, loserId, won) {
                fetch('/notify-result', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
            ...
            window.addEventListener('beforeunload', function (e) {
                ...
                db.collection('online-users').doc(userId).delete().then(() => {
                    db.collection('users').doc(userId).get().then((userDoc) => {
                        ...
                        if (userData.friends && userData.friends.length > 0) {
                            const friendId = userData.friends[0]; // Assume only one friend for simplicity
                            notifyFriend(friendId, userId, false);
                        }
                    });
                });
            });

document.addEventListener('DOMContentLoaded', function () {
    ...
    const serverUrl = 'https://codeduels-04ddc5bd753d.herokuapp.com';
        ...
        require(['vs/editor/editor.main'], function () {
            ...
            firebase.auth().onAuthStateChanged((user) => {
                if (user) {
                    ...
                    async function notifyFriend(userId, won, message, type) {
                        try {
                            const userDoc = await db.collection('users').doc(userId).get();
                            const userData = userDoc.data();
                            if (userData.friends && userData.friends.length > 0) {
                                const friendId = userData.friends[0]; // Assume only one friend for simplicity
                                const response = await fetch(`${serverUrl}/notify-result`, {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json'
                                    },
                                    body: JSON.stringify({ userId: friendId, won: !won })
                                });
